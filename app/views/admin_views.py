...
from app import db
from app.models.user_models import Courses, User, Role, UsersRoles
...
def admin_list_courses():
    #courses = Courses.query.order_by(Courses.course_name.asc())
    #for course in courses:  , courses=courses
        #print(course.course_name)
    return render_template('admin/admin_list_courses.html') 

@admin_blueprint.route('/admin/create_courses', methods=['GET', 'POST'])
@roles_required('admin')  
def admin_create_courses():
    
    if request.method == 'POST':
        if len(request.form['course_name']) < 3:
            flash('Role name too short', 'error')
        else:
            course = Courses()
    ...
    return render_template('admin/admin_create_edit_courses.html', verb="Create")

@admin_blueprint.route('/admin/delete_course/<courses_id>')
@roles_required('admin')  
def admin_delete_course(courses_id):
    course = Courses.query.filter(Courses.id == courses_id).first()
    db.session.delete(course)
    db.session.commit()
    flash('Course Deleted!!', 'success')
    return redirect(url_for('admin.admin_list_courses'))

@admin_blueprint.route('/admin/edit_courses/<courses_id>', methods=['GET', 'POST'] )
@roles_required('admin')  
def admin_edit_courses(courses_id):
    course = Courses.query.filter(Courses.id == courses_id).first()
    if request.method == 'GET':
        request.form.course_name = course.name
    elif request.method == 'POST':
        if len(request.form['course_name']) < 3:
            flash('Role name too short', 'error')
        else:
            ...
            return redirect(url_for('admin.admin_list_courses'))
    return render_template('admin/admin_create_edit_courses.html', verb="Edit")
